...
{	
	...
	public void GetTopKTreesForBody (Atom key, Body b, List<DerivationTree> topKTrees)
	{
		...
		for (int i = 0; i < this.k; i++) 
		{
			...
			if (chosenNode.getWeight() > 0)
			{
				...
				if (topKTrees.size() < this.k || topKTrees.get(topKTrees.size() - 1).getWeight() < chosenNode.getWeight()) 
				{
	...
	public DerivationTree InitializeTreeOfCompareNode (Atom key, Vector<List<DerivationTree>> arr, ComparissonTree.CompareNode v, Body body) 
	{
	...
	public void InitializeCompareNode (Atom key, Vector<List<DerivationTree>> arr, ComparissonTree.CompareNode v, Body body) 
	{
		...
		if (true == validTree && (false == this.online || key.getTrees() == null || key.getTrees().get(0).getWeight() < weight)) //confirm it is not empty tree
		{
			v.setWeight(weight);
		}
	}
	...
	public void UpdateTop1WhileSemiNaive(Atom key, Body body)
	{
	...
	public void SiftTopKTrees (Atom key, List<DerivationTree> topKTrees)
	{
		if (false == topKTrees.isEmpty())
		{
			...
			else if (null == key.getTrees() || false == key.getTrees().equals(topKTrees)) 
			{
				if (null != key.getTrees()) 
				{
					topKTrees.addAll(key.getTrees());
	...
	public void UpdateForTop1 (Atom key, List<DerivationTree> topKTrees)
	{
	...
	public void Update (Atom key, List<DerivationTree> topKTrees)
	{
