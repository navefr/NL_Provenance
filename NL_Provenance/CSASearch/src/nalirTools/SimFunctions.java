...
public class SimFunctions 
{
	...
	public static void similarity(ParseTreeNode treeNode, MappedSchemaElement element)
	{
		...
		else
		{
			...
			{
				String mappedValue = mappedValues.get(i); 
				String [] words = mappedValue.split("###"); 
				mappedValues.set(i, words[1]); 
				
				sims[i] = SimFunctions.pqSim(nodeLabel, words[1]); 				
				if(words.length > 2)
				{
					mappedValues.set(i, words[1] + " (#" + words[2] + ")"); 
					if(SimFunctions.pqSim(nodeLabel, words[1]) > sims[i])
					{
						sims[i] = SimFunctions.pqSim(nodeLabel, words[1]); 
					}
				}
				
				double importance = Double.parseDouble(words[0]) / 21; 
				sims[i] = Math.sqrt(((importance*sims[i] + sims[i])/2)); 
			}			
